...
namespace kl
{
    public partial class publish : Form
    {
        ...
        public publish(int ids)
        {
            InitializeComponent();

            int[] coalesce = request.getPublishingCoalesce(ids);
        ...
        public void readlist()
        {
            ...
            if (pubList.Count > 0)
            {
                ...
                {
                    dataGridView1.Rows.Add(pubList[i].id, pubList[i].name);
                }
            }
            else
            {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (textBox1.Text.Count() > 0)
            {
                ...
                string response = request.addPublishing(name);
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            {
                request.updatePublishing(dataGridView1[1, Convert.ToInt32(edl[i])].Value.ToString(), dataGridView1[0, Convert.ToInt32(edl[i])].Value.ToString());
            }
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            try
            {
                request.delPublishing(dataGridView1[0, dataGridView1.CurrentRow.Index].Value.ToString());
